...
Create a `.env.runner` file in the PredRet directory:
...
```bash
# Navigate to your PredRet directory
cd /path/to/PredRet
...
Edit `docker-compose.runner.yml` and increase the resource limits:
```yaml
deploy:
  resources:
    limits:
...
```bash
# Start runner 1
docker-compose -f docker-compose.runner.yml --env-file .env.runner -p runner1 up -d

# Start runner 2 (with a different token)
docker-compose -f docker-compose.runner.yml --env-file .env.runner2 -p runner2 up -d
```

For each runner:
1. Get a NEW token from https://github.com/settings/actions/runners
2. Create a new `.env.runner2`, `.env.runner3`, etc.
3. Give each a different `RUNNER_NAME` (e.g., docker-runner-1, docker-runner-2)
4. Start with different project names: `-p runner1`, `-p runner2`
