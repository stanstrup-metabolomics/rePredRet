...
.find_data_dir <- function(subdir) {
  candidates <- c(
    file.path("website", "data", subdir),
    file.path("..", "website", "data", subdir),
    file.path("data", subdir)
  )
  for (d in candidates) if (dir.exists(d)) return(d)
  return(file.path("website", "data", subdir))
}
stats_dir <- .find_data_dir("statistics")
models_dir <- .find_data_dir("models")
...
- [Model Statistics](models.html) - Detailed model performance metrics
- [Predictions](predictions.html) - Prediction coverage and CI widths
- [Network](network.html) - System connectivity visualization
- [Methods](methods.html) - Algorithm documentation
